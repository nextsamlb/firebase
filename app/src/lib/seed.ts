...
import type { Player, Match, MediaItem, AppSettings, League } from './data';
...
async function seedDatabase() {
    try {
        ...
        console.log('Clearing existing players, matches, app_settings, leagues...');
        await clearMediaItems();
        console.log('Cleared mediaHubItems.');

        const collectionsToClear = ['players', 'matches', 'app_settings', 'leagues'];
        for (const col of collectionsToClear) {
            const ref = collection(db, col);
            const snapshot = await getDocs(ref);
            snapshot.docs.forEach((doc) => batch.delete(doc.ref));
        }
        console.log('Cleared existing data from players, matches, app_settings, leagues.');
        ...
        // Add a default League
        console.log('Preparing default league data...');
        const leagueDocRef = doc(collection(db, 'leagues'));
        ...
        batch.set(leagueDocRef, newLeague);
        console.log('Prepared default league.');
        ...
        console.log(`   - Added 1 default league.`);
